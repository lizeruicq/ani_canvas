...
export default function PropertiesPanel({ width }: { width: number }) {
  ...
  const TabButton = ({ id, label }: { id: 'props' | 'code'; label: string }) => (
    <button
      onClick={() => setTab(id)}
      className={[
        'px-2 py-1 text-[11px] font-medium rounded-md transition-colors',
        tab === id ? 'bg-accent/15 text-accent' : 'text-muted hover:text-text hover:bg-panel2',
      ].join(' ')}
    >
      {label}
    </button>
  ...
  return (
    <div className="flex flex-col border-l border-edge bg-panel1 shrink-0 overflow-y-auto" style={{ width }}>
      <div className="h-10 flex items-center justify-between px-3 border-b border-edge shrink-0">
        <div className="flex items-center gap-1.5">
          <TabButton id="props" label="属性" />
          <TabButton id="code" label="代码" />
        </div>
        {tab === 'props' && node && (
